...
//   // Build data for backend (⚡ no name or index, backend handles it)
//   const newItem = {
//     type: formData.itemType,
//     pipeType: formData.pipeType,
//     guage: isPillars ? "" : String(formData.guage || ""),
...
export default function ItemCard({ initialData }: ItemCardProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ✅ Clean payload for backend (backend decides `name` + merging)
    const newItem = {
      name: formData.itemName, // ✅ for Hardware & Pillars only
      type: formData.itemType,
      pipeType: formData.pipeType,
      size: formData.itemSize,
      guage: formData.guage,
